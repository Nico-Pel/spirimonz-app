...
namespace YsoCorp {
    ...
    public static class ADataManager {
        ...
        // DICTIONARY
        public static Dictionary<T1, T2> GetDictionary<T1, T2>(string key, string value = "[]") {
            List<Tuple<T1, T2>> temp = GetObject<List<Tuple<T1, T2>>>(key, value);
            return temp.ToDictionary((t) => t.Item1, (t) => t.Item2);
        }
        public static void SetDictionary<T1, T2>(string key, Dictionary<T1, T2> value) {
            List<Tuple<T1, T2>> temp = new List<Tuple<T1, T2>>();
            foreach (var kvp in value) {
                temp.Add(new Tuple<T1, T2>(kvp.Key, kvp.Value));
            }
            SetObject(key, temp);
        }
